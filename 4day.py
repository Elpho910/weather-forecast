...
OUTPUT_TXT_PATH = os.path.join(SCRIPT_DIR, "4day-forecast.txt")

# Configure logging
LOG_PATH = os.path.join(SCRIPT_DIR, "4day-forecast.log")
...
def parse_forecast(xml_file):
    ...
    # Process forecast for the selected area
    for area in root.findall(".//area"):
        ...
        if desc == LOCATION:
            ...
            for period in forecast_periods:
                period_label = period.attrib.get("index") or period.attrib.get(
                    "start-time-local"
                )
                ...
                try:
                    ...
                    readable_date = start_time.strftime("%A (%d %B)")
                except ValueError:
                    readable_date = f"Period starting {start_time_str}"
                    logger.warning(
                        f"Could not parse start-time-local: {start_time_str}"
                    )

                forecast_lines.append(f"{readable_date}:")
                for t in period.findall("text"):
                    t_type = t.attrib.get("type", "")
                    t_value = (t.text or "").strip()
                    if t_value:
                        readable_label = t_type.replace("_", " ").capitalize()
                        logger.info(f"{readable_label}: {t_value}")
                        forecast_lines.append(f"{readable_label}: {t_value}")
                forecast_lines.append("")  # blank line between days

            logger.info("Successfully extracted multi-day forecast.")
            return "\n".join(forecast_lines)
